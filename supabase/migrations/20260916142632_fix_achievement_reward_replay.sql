-- LIVE-12 fix: achievement rewards could be claimed repeatedly.
--
-- verify_achievement_progress only gated payout on whether a
-- user_achievements row was CURRENTLY unlocked. Since user_achievements
-- was client-writable (own-row DELETE/INSERT/UPDATE grants + a FOR ALL
-- policy), a client could delete their own unlocked row and re-call the
-- RPC: progress re-derives as met, the row is reinserted, and XP/coins
-- are paid out again. Verified live 2026-09-15/16 (rolled back).
--
-- Fix has two independent layers, root-caused in the one shared function
-- every achievement (except feeling_lucky, which never reaches this
-- function -- see award_achievement_rewards) pays out through:
--
--  1. Ledger-based idempotency: xp_transactions/coin_transactions already
--     carry a reference_id, and this function (and every other reward
--     path in the app -- checkin rewards, invite rewards) already uses
--     'achievement_<id>' / '<ref>' as that tie-back key. Check it before
--     awarding, not just whether the row is presently unlocked. This
--     mirrors the exact pattern award_achievement_rewards already uses
--     for feeling_lucky.
--  2. Partial unique indexes on (user_id, reference_id) for
--     achievement_-prefixed rows close the race window between the check
--     and the award (defense in depth on a money path).
--
-- Root cause closed: user_achievements is locked to SELECT-only for the
-- client. All legitimate writes already go through this SECURITY DEFINER
-- function, which upserts as the function owner -- the client never
-- needed direct table write access.

-- ── 1. Close the client write path on user_achievements ────────────────
...
-- ── 2. Race-condition hardening on the reward ledger ────────────────────
-- No existing achievement_-prefixed reference_id has ever repeated for a
-- user (verified live before applying), so this applies cleanly.
create unique index if not exists xp_transactions_achievement_ref_uniq
  on public.xp_transactions (user_id, reference_id)
  where reference_id like 'achievement_%';

create unique index if not exists coin_transactions_achievement_ref_uniq
  on public.coin_transactions (user_id, reference_id)
  where reference_id like 'achievement_%';

-- ── 3. Ledger-checked idempotency in verify_achievement_progress ───────
...
BEGIN
  ...
  -- LIVE-12 fix: the user_achievements row above is not a safe payout
  -- gate on its own -- it's client-deletable (was; see policy/grant
  -- changes above), so "currently unlocked" can be made false again and
  -- this function re-called. Check the reward ledger itself, keyed by
  -- the same reference_id convention already used everywhere else in
  -- this codebase (checkin rewards, invite rewards, and the
  -- feeling_lucky branch of award_achievement_rewards) before paying.
  IF EXISTS (
    SELECT 1 FROM xp_transactions
    WHERE user_id = v_caller AND reference_id = 'achievement_' || p_achievement_id
    UNION ALL
    SELECT 1 FROM coin_transactions
    WHERE user_id = v_caller AND reference_id = 'achievement_' || p_achievement_id
  ) THEN
    RETURN jsonb_build_object('unlocked', true, 'already_awarded', true);
  END IF;
...
revoke execute on function public.verify_achievement_progress(text) from public, anon;
